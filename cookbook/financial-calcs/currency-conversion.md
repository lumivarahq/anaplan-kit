...
## Blueprint
**`SYS30 Entity Details`** — each entity's currency, `Applies To` Entity:

| Line Item | Format | Summary | Applies To | Formula |
| --- | --- | --- | --- | --- |
| Local Currency | List: Currency | None | Entity | *(mapping/input)* |
...
**`CAL100 Reporting Values`** — `Applies To` Entity × Time:

| Line Item | Format | Summary | Applies To | Formula |
| --- | --- | --- | --- | --- |
| Local Amount | Number | Sum | Entity, Time | *(from source, in local ccy)* |
| FX Rate | Number | None | Entity, Time | `INP40 FX Rates.Average Rate[LOOKUP: SYS30 Entity Details.Local Currency]` |
| Reporting Amount (USD) | Number | Sum | Entity, Time | `Local Amount × FX Rate` |
...
```
// CAL100 -> FX Rate
INP40 FX Rates.Average Rate[LOOKUP: SYS30 Entity Details.Local Currency]
```
...
```
INP40 FX Rates.Closing Rate[LOOKUP: SYS30 Entity Details.Local Currency]
```
...
- Don't hard-code currency with `SELECT: Currency.USD` — map via `SYS30` (*Sustainable*).
- Aggregating converted amounts up the entity hierarchy is fine **after** conversion; never sum local amounts of different currencies before converting.
