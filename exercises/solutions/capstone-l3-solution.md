...
| `L3 Cost Centre/Entity` | hierarchy leaf | `L2 Country` | — | CC-1100 UK Sales, … |
...
Planning grain (leaf lists): **`L2 Product`** and **`L3 Cost Centre/Entity`**. *(AC1)*
...
| --- | --- | --- | --- |
| `SYS01 Time Settings` | System | Time | `Is Actual Month?`, dates, year flags |
| `SYS02 Product Details` | System | L2 Product | `COGS %`, family map |
| `SYS03 Org Details` | System | L3 Cost Centre | `Country`, `Region`, `Local Currency`, `Is Active?` |
| `SYS04 Exchange Rates` | System | Currency × Time × Versions | `Rate to Group` (filled), `Is Group Currency?` |
...
| `INP03 Cost Drivers` | Inputs | L3 CC × Time × Versions | `Fixed OpEx (Local)` |
| `CAL01 Price` | Calculations | L2 Product × Time × Versions | derived monthly `Price` |
| `CAL02 Revenue` | Calculations | L3 CC × L2 Product × Time × Versions | `Gross Revenue (Local)` |
| `CAL03 Costs` | Calculations | L3 CC × L2 Product × Time × Versions | `COGS (Local)` |
...
| `OUT01 P&L Report` | Outputs | L3 CC × Time × Versions | report (Local/USD selectable) |
| `OUT02 Top Products` | Outputs | L2 Product × Time | revenue + rank |

```
INP02 Price Setup ─> CAL01 Price ─┐
INP01 (Volume,override) ──────────┼─> CAL02 Revenue ─┐
SYS02 (COGS %) ───────────────────┘                  ├─> CAL04 P&L (Local) ─> CAL05 P&L (USD) ─┐
INP03 (Fixed OpEx) ──────────────> CAL03 Costs ──────┘            ▲                            ├─> CAL06 Reported ─> OUT01
SYS04 (FX) + SYS03 (Local Currency) ─────────────────────────────┘                            │
DAT01 Actuals ────────────────────────────────────────────────────────────────────────────────┘ (blend via SYS01.Is Actual Month?)
```
...
| --- | --- | --- | --- |
| `Years From Base` | Number | None | `ITEM(Time.Year) ... ` → `YEARVALUE(Time) - YEARVALUE(START of model)` (count of years since the base year, derived from the calendar — no year named) |
...
| `Price` | Number | Average | `IF ISNOTBLANK('INP01 Revenue Assumptions'.Price Override) THEN 'INP01 Revenue Assumptions'.Price Override ELSE Derived Price` |
...
```
Gross Revenue (Local) = 'INP01 Revenue Assumptions'.Volume * 'CAL01 Price'.Price
```
**`CAL03 Costs`** · same grain:
```
COGS (Local) = 'CAL02 Revenue'.Gross Revenue (Local) * 'SYS02 Product Details'.COGS %
```
COGS % from **System**, not editable on the input page. *(AC4)*
...
| `COGS` | Sum | `'CAL03 Costs'.COGS (Local)[SUM: L2 Product]` |
...
**`SYS04 Exchange Rates`** holds `Rate to Group` per Currency × Time × Versions (filled to 1 for
USD). **`CAL05 P&L (USD)`** converts each line, looking up the rate by the cost centre's local
currency:
```
Revenue USD = 'CAL04 P&L (Local)'.Revenue
            * 'SYS04 Exchange Rates'.Rate (filled)[LOOKUP: 'SYS03 Org Details'.Local Currency]
```
...
```
(Repeat per line.) Keyed to `Is Actual Month?` (driven by the current period) — advancing the period
...
```
Revenue           = 'CAL02 Revenue'.Gross Revenue (Local)[SUM: L3 Cost Centre/Entity]
Product Rev Rank  = RANK(Revenue, DESCENDING, ANY, L2 Product)   // within each month
