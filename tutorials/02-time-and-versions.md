...
model: **Time** and **Versions**. Almost every module in our FP&A model uses both. We use the same
calendar as the [blueprint](../blueprints/_common/time-and-versions.md).
...
2. Set:
   ...
   - **Current Period:** `Mar 2025` (our "today" for this build — it splits Actual from Forecast
     later).
   - **Fiscal year start month:** January (FY = calendar year here, to keep it simple).
3. Set the **range of years** to span **FY2024 → FY2026** (one actual-ish year plus plan years).
   This yields **FY24, FY25, FY26** with months and quarters under each.
...
> assumptions module only spans the plan years. For now, the model calendar is fine.
...
> current period** (`Mar 2025`). Then any line item with *Time Range = model calendar* and the
> right setup reads Actual ≤ Feb 2025 and Forecast from Mar 2025 on. We'll wire the logic
...
- [ ] Time shows **FY24 / FY25 / FY26**, each with 4 quarters and 12 months.
- [ ] Current Period = `Mar 2025`.
