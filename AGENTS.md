...
pip install -e ".[dev]"          # runtime + test deps (pytest, responses)
# from repo ROOT:
...
- `tooling/` — `anaplan_kit` pkg (auth, client, metadata, imports_exports, actions) + `modeling/` CLI (`anaplan-model` entrypoint) + offline tests.
...
- Tests mock HTTP (`responses`) — never make live calls in tests.
